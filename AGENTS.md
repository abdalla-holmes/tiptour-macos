...
macOS menu bar voice companion. Lives entirely in the macOS status bar (no dock icon, no main window). Clicking the menu bar icon opens a custom floating panel. Voice uses one realtime Gemini Live path by default; Pipecat is tracked as an optional local voice harness sidecar for future realtime voice runtime work. Ctrl+K text commands can route to Hermes or Claude for one-action planning through the same TipTour grounding/action engine.
...
- **Source Layout**: TipTour source is grouped by responsibility: `App/` for lifecycle and `CompanionManager`, `Core/` for the stable `TipTourEngine` facade, `Actions/` for action-driver boundaries, `Workflow/` for single-action plans and grounding, `Perception/` for screen/AX/DOM/OCR/local detection plus optional parser harness clients, `Harnesses/` for local external transports, `Plugins/` for connection metadata, `Skills/` for portable markdown app skills plus tiny runtime hints, `UI/` for panel/overlay views, `Voice/` for Gemini Live and optional voice harness clients, `Recording/` for walkthrough capture, and `Utilities/` for cross-cutting helpers. Keep general agent/source-map docs outside `TipTour/`; markdown under `TipTour/Skills/**/SKILL.md` is intentionally bundled as app-skill content.
...
- **Pipecat Voice Harness**: Pipecat is treated as an optional local voice sidecar harness, not as a replacement for TipTour's pointer/action engine. TipTour exposes a persisted "Pipecat Voice" connection toggle and a lightweight `PipecatVoiceHarnessClient` contract for a sidecar at `127.0.0.1:7860`. The sidecar should call TipTour's localhost harness for observe/targets/actions and may delegate long-horizon work to Hermes at `127.0.0.1:8642`; see `docs/pipecat-harness.md`.
- **OmniParser Harness**: OmniParser is treated as an optional local parser sidecar, not as a replacement for the native detector yet. TipTour exposes a persisted "OmniParser" connection toggle and a lightweight `OmniParserHarnessClient` contract for the existing local sidecar at `127.0.0.1:8765` (`/Users/milindsoni/Documents/mywork/OmniParser/server.py`). When enabled, TipTour still runs native YOLO/OCR, asks OmniParser to parse the same cursor-screen screenshot, and merges parser elements into `LocalPerceptionTargetCache`; see `docs/omniparser-harness.md`.
...
- **Action Grounding**: Gemini calls `submit_workflow_plan`; click-like steps are grounded by `ElementResolver`, which resolves exact local `target_id`/`target_mark` first when present, then falls back to the step `label` via a local-first lookup. Gemini may provide `point_2d` (`[y, x]`, normalized to 0-1000) and/or `box_2d` (`[y1, x1, y2, x2]`). TipTour prefers deterministic local geometry when available and only falls back to Gemini coordinates after local resolvers miss.
    ...
    3. **Local perception cache (experimental branch only)** — the native overlay publishes the latest CoreML UI detections plus Apple Vision OCR into `LocalPerceptionTargetCache`, and can append optional OmniParser sidecar results when enabled. This lets labels such as "Add" resolve without screenshot streaming or Gemini `box_2d`.
...
- **Connection Toggles**: The menu bar panel has a small "Connections" section for user-controlled plugin-like integrations. `CUA Driver` defaults ON and gates all desktop action execution through `ActionExecutor`; when OFF, TipTour can still observe/ground but action plans are rejected before execution. `Hermes Auto` defaults OFF; when ON, only long-running Ctrl+K prompts are delegated to the local Hermes API server while simple pointer actions and Claude one-step planning remain available. `OmniParser` defaults OFF and appends optional parser sidecar results to the local target graph. `Pipecat Voice` defaults OFF and records intent to use a local realtime voice sidecar while Gemini remains the default in-app voice runtime.
...
| `TipTour/Plugins/TipTourConnection.swift` | ~27 | Lightweight built-in connection model for plugin-like integrations such as CUA, Hermes, perception, parser sidecars, and harnesses. |
| `docs/omniparser-harness.md` | ~71 | OmniParser parser sidecar architecture and response contract. Defines how OmniParser output is merged into TipTour's local target graph. |
| `docs/pipecat-harness.md` | ~71 | Pipecat sidecar architecture and tool contract. Defines how Pipecat should call TipTour tools directly and delegate longer work to Hermes. |
...
| `TipTour/Perception/LocalPerceptionTargetCache.swift` | ~449 | Shared local target cache. Stores YOLO/OCR and optional OmniParser detections, enriches unlabeled UI boxes with OCR labels, exposes live grounding targets with stable marks/IDs to the harness, strictly matches spoken labels, and converts screenshot-pixel boxes into global AppKit coordinates. |
...
| `TipTour/Perception/OmniParserHarnessClient.swift` | ~135 | Optional local OmniParser parser sidecar client. Health-checks `127.0.0.1:8765` and parses cursor-screen screenshots into elements that can be merged into `LocalPerceptionTargetCache`. |
...
| `TipTour/Voice/PipecatVoiceHarnessClient.swift` | ~65 | Lightweight local Pipecat sidecar contract. Provides health probing and shared tool instructions for TipTour/Hermes interoperability. |
...
| `TipTour/Utilities/TipTourDefaults.swift` | ~143 | Centralized UserDefaults facade for persisted app preferences and first-run defaults such as Autopilot, CUA, Hermes/OmniParser/Pipecat connection toggles, screenshots, panel pinning, and permission memory. |
