...
- `TipTour/Perception/` owns screen capture, AX, DOM/CDP, local detection, OCR, optional parser sidecar clients, and target caches.
...
- TipTour does not have a dynamic plugin marketplace yet. Keep the model explicit and boring: built-in connections such as CUA, Hermes, OmniParser, and Pipecat Voice are represented by small models, toggled from the menu bar, and enforced by the engine/action facade.
